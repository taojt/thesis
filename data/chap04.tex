...
\begin{table*}[htbp]
  \caption{基于CGAN 分割方法的生成器分割模型详细参数}\label{tab:model_g_param}
  \centering
  \makebox[\textwidth]{
    \begin{adjustbox}{max width=1.1\textwidth}
      \begin{tabular}{c|c|cccccc}
        \toprule
                                 & 模块                       & Levels                    & 网络层     & 该层输出尺寸                & 卷积核（尺寸/通道）  & 步长 & 激活函数 \\
        \midrule
        ...
        \cline{2-8}
                                 & G-输出                     & level 10                  & conv10\_1  & $256\times 256\times 1   $  & $    1\times 1/1  $  & 1    & ReLU     \\
        \midrule
        \multirow{10}*{判别器D } & D-输入                     &                           &            & $256\times 256\times 5   $  &                      &      &          \\
        \cline{1-8}
                                 & G-输出                     &                           & conv10\_1  & $1\times 1   $              &                      &      & Sigmoid  \\
        \bottomrule
      \end{tabular}
    \end{adjustbox}}
\end{table*}
