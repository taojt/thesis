...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}[1]
   ...
   \textbf{模型训练：}
    \State 预训练VGGNet16 模型权值初始化生成模型，判别器步数 $k= 1$，权衡因子$\lambda$, 初始学习率$\alpha$, Adam 动量 $\beta_1,\beta_2$。
    \For{模型迭代epoch 数}
        \State \textbf{判别器：}
        \For{$k$ 步 }
