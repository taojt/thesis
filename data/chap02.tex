...
本文主要从遥感影像不确定性数据建模和地物分类一致性两个角度对高分影像分类识别展开研究。第~\ref{cha:chap03} 章研究内容是对面向对象的模糊聚类影像无监督分割方法改进，这部分工作基于遥感影像的模糊聚类分割基础知识。第~\ref{cha:chap04} 章则是针对全卷积分割网络中边界模糊、类内不一致性问题，提出基于条件生成对抗网络框架的影像分割方法，该章节中大量工作基于卷积神经网络和全卷积语义分割网络模型相关知识。所以本章将依次介绍遥感影像模糊聚类分割方法，深度神经网络，全卷积分割网络相关的理论知识。
...
迭代停止时，样本$x_i$ 对所有聚类中心$[c_1,c_2,\cdots,c_j,\cdots,c_m]$ 的隶属度矩阵向量为$U_{\cdot i} = [\mu_{1i},\mu_{2i},\cdots,\mu_{ji},\cdots,\mu_{mi}]$，样本$x_i$ 最终所属类别依据隶属度最大化原则划分为类别$c_K$，则有：
\begin{equation}
  \label{eq:2-14}
